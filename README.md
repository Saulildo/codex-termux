...
- Upstream base: OpenAI Codex `rust-v0.144.0`, packaged as `0.144.0` on npm `next`. The `latest` dist-tag remains on `0.142.5` until a separate promotion; `stable` stays on `0.140.0` for conservative installs.
...
- Latest runtime validation report: 0.144.0 next validation pending; previous report [v0.142.0 Termux device validation](./test-report/CODEX_TEST_REPORT_v0.142.0_run_20260623-2007.md)
